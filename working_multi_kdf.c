...
u8 roundKeys[NUM_THREADS][11][4][4];
...
void RijndaelKeySchedule( const u8 const key[16],unsigned int thread_id )
{
  u8 roundConst;
  int i, j;
  ...
  for (i=0; i<16; i++)
    roundKeys[thread_id][0][i & 0x03][i>>2] = key[i];

  roundConst = 1;

  /* now calculate round keys */
  for (i=1; i<11; i++) {
    roundKeys[thread_id][i][0][0] = S[roundKeys[thread_id][i-1][1][3]]
                         ^ roundKeys[thread_id][i-1][0][0] ^ roundConst;
    roundKeys[thread_id][i][1][0] = S[roundKeys[thread_id][i-1][2][3]]
                         ^ roundKeys[thread_id][i-1][1][0];
    roundKeys[thread_id][i][2][0] = S[roundKeys[thread_id][i-1][3][3]]
                         ^ roundKeys[thread_id][i-1][2][0];
    roundKeys[thread_id][i][3][0] = S[roundKeys[thread_id][i-1][0][3]]
                         ^ roundKeys[thread_id][i-1][3][0];

    for (j=0; j<4; j++) {
      roundKeys[thread_id][i][j][1] = roundKeys[thread_id][i-1][j][1] ^ roundKeys[thread_id][i][j][0];
      roundKeys[thread_id][i][j][2] = roundKeys[thread_id][i-1][j][2] ^ roundKeys[thread_id][i][j][1];
      roundKeys[thread_id][i][j][3] = roundKeys[thread_id][i-1][j][3] ^ roundKeys[thread_id][i][j][2];
    }
...
void KeyAdd(u8 state[4][4], u8 roundKeys[][11][4][4], int round,unsigned int thread_id)
{
  int i, j;

  for (i=0; i<4; i++)
    for (j=0; j<4; j++)
      state[i][j] ^= roundKeys[thread_id][round][i][j];
 ...
 * key).
 *-----------------------------------------------------------------*/
void RijndaelEncrypt( const u8 const input[16], u8 output[16],unsigned int thread_id )
{
  u8 state[4][4];
  int i, r;
  ...
  /* add first round_key */
  KeyAdd(state, roundKeys, 0,thread_id);
  ...
  for (r=1; r<=9; r++) {
    ...
    ShiftRow(state);
    MixColumn(state);
    KeyAdd(state, roundKeys, r,thread_id);
  }
          ...
          uint8_t mac_a[8],unsigned int thread_id )
{
  ...
  uint8_t i;
  RijndaelKeySchedule( k,thread_id);

  for (i=0; i<16; i++)
    rijndaelInput[i] = _rand[i] ^ opc[i];

  RijndaelEncrypt( rijndaelInput, temp ,thread_id);
  ...
  RijndaelEncrypt( rijndaelInput, out1,thread_id );
             ...
             uint8_t res[8], uint8_t ck[16], uint8_t ik[16], uint8_t ak[6] ,unsigned int thread_id)
{
  ...
  rijndaelInput[15] ^= 1;
  RijndaelEncrypt( rijndaelInput, out,thread_id );
                    ...
                    uint8_t sqn[6], auc_vector_t *vector,unsigned int thread_id)
{
  ...
  /* Compute MAC */
  f1(opc, key, vector->rand, sqn, amf, mac_a,thread_id);
  ...
  f2345(opc, key, vector->rand, vector->xres, ck, ik, ak,thread_id);
...
void *multithread_gen_vec(void *thread_id)
{
  unsigned int *tid= (unsigned int *)thread_id ;
  unsigned int thread_index, data_start,data_end,data_size,remaining_data;
  int i=0;
  thread_index=*tid;
  data_size=NUM_REQUESTS/NUM_THREADS;
  data_start=thread_index*data_size;
  data_end = (thread_index*data_size) + data_size;

if(NUM_REQUESTS%NUM_THREADS!=0 && thread_index==NUM_THREADS-1)
{
   remaining_data=NUM_REQUESTS/NUM_THREADS + (NUM_REQUESTS%NUM_THREADS);
   data_end = (thread_index*data_size) + remaining_data;
}
//  printf("thread_id is %u, start_data %u, end_data %u, data size %u\n",thread_index,data_start,data_end,data_size);
  for(i=data_start;i<data_end;i++)
    generate_vector(input_data[i].opc_in, input_data[i].imsi_in, input_data[i].key_in,input_data[i].plmn_in,input_data[i].sqn_in, input_data[i].auc_vector_in,thread_index);
}
...
*/  
  ...
  input_data = (input_request*)malloc(NUM_REQUESTS*sizeof(input_request));
  for(i=0;i<NUM_REQUESTS;i++)
    init_input(&input_data[i],i);
...
*/
  ...
  {
    thread_id[i]=i;   
    pthread_create(&threads[i],NULL,multithread_gen_vec, &thread_id[i]);
  }
...
/*
  for(i=0;i<NUM_REQUESTS;i++)
  {
    printf("AUC_VECTOR_RAND %u\n",input_data[i].auc_vector_in->rand_new);
    print_buffer("XRES    : ", input_data[i].auc_vector_in->xres, 8);
    print_buffer("AUTN    : ", input_data[i].auc_vector_in->autn, 16);
    print_buffer("KASME    : ", input_data[i].auc_vector_in->kasme, 16);
  }
*/
